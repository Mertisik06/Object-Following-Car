...
void GPIOPortB_UltrasonicTask(void)
{
	if(GPIO_PORTB_RIS_R & GPIO_PORTB_PB5_M)    // PB5 interrupt occurred
	{
		...
		{
			ult2.change = (ult2.second_time ) / 1000.0;  // time change in ms
			ult2.dist = (ult2.second_time ) / 58.82;     // calculate distance in cm
			ult2.distMeasure = 0;
...
{ 
	if(GPIO_PORTE_RIS_R & GPIO_PORTE_PE0_M)    // PE0 interrupt occurred
	{
		...
		{
			ult1.change = (ult1.second_time ) / 1000.0; // time change in ms
			ult1.dist = (ult1.second_time ) / 58.82;    // calculate distance in cm
			ult1.distMeasure = 0;
